# Binary search
Binary search is used to search for a specific member inside an ascending order array. This search method has the time complexity of O(logn) which is faster than the classicial method to iterate through every member of the array, which results in O(n).
